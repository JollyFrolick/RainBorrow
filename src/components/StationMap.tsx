...
export default function StationMap({ stations, selectedId, onSelect, location, mode, centerRequest, onLocate }: StationMapProps) {
  ...
  return <View style={{ flex: 1, minHeight: 280 }}><MapView ref={ref} style={StyleSheet.absoluteFill} initialRegion={{ latitude: 22.2835, longitude: 114.1565, latitudeDelta: .014, longitudeDelta: .014 }} showsUserLocation={!!location} showsMyLocationButton={false}>
  ...
  </MapView><View style={{ position: 'absolute', right: 12, top: 12, gap: 8 }}><IconButton icon="navigation" label="Show my location" onPress={onLocate} /><IconButton icon="maximize" label="Show all stations" onPress={() => { if (stations.length) ref.current?.fitToCoordinates(stations, { edgePadding: { top: 50, bottom: 50, left: 50, right: 50 }, animated: true }); }} /></View></View>;
}
